...
test("首页摘要会展示今日健康记录、今日跑步记录和目标状态", () => {
  ...
  assert.deepEqual(
    ...
    [
      ["健康打卡", "已记录", "/records"],
      ["跑步打卡", "已记录 2 次", "/records"],
      ["提醒状态", "未开启提醒", "/settings"],
      ["目标设置", "已设置", "/#goal-settings"],
    ],
  );
...
test("首页摘要会计算健康曲线、跑步曲线和 BMI 指标", () => {
  ...
  assert.equal(runDefaultOption.metrics[0].points.length, 2);
  assert.equal(runDefaultOption.metrics[0].points.at(-1).value, 5);
  assert.equal(runDefaultOption.metrics[2].value, "6");
  assert.equal(runMonthOption.metrics[0].points.length, 2);
});
