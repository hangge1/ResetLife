...
test("首页摘要在空数据时返回中文引导状态", () => {
  ...
  assert.equal(summary.chartPanels[0].metrics[0].label, "体重");
  assert.equal(summary.chartPanels[1].metrics[0].label, "每日跑量");
  assert.equal(summary.trendCards[0].value, "数据不足");
  assert.equal(summary.trendCards[4].value, "未计算");
  assert.match(summary.trendCards[4].description, /需要先在设置中填写身高/);
});

test("首页摘要会计算健康趋势、跑步趋势和 BMI", () => {
  const { recordsRepository, goalsRepository } = createMockRepositories({
    latestHealth: [
      {
        id: "health-new",
  ...
  assert.equal(summary.trendCards[0].title, "健康趋势");
  assert.equal(summary.trendCards[0].periodLabel, "最近 7 天");
  assert.equal(summary.trendCards[0].value, "-1.2");
  assert.equal(summary.trendCards[0].unit, "公斤");
  assert.match(summary.trendCards[0].description, /体重变化 -1.2 公斤/);
  assert.equal(summary.trendCards[2].title, "跑步趋势");
  assert.equal(summary.trendCards[2].value, "9.5");
  assert.equal(summary.trendCards[2].unit, "公里");
  assert.match(summary.trendCards[2].description, /记录 2 次跑步/);
  assert.equal(summary.trendCards[4].title, "BMI");
  assert.equal(summary.trendCards[4].value, "24.5");
  assert.match(summary.trendCards[4].description, /身高 175 厘米/);
  assert.equal(summary.chartPanels[0].periodLabel, "最近 30 天");
  assert.deepEqual(
    summary.chartPanels[0].metrics.map((metric) => metric.label),
    ["体重", "腰围", "体脂率", "BMI"],
  );
  assert.equal(summary.chartPanels[0].metrics[0].value, "75");
  assert.equal(summary.chartPanels[0].metrics[0].change, "变化 -1.2 公斤");
  assert.equal(summary.chartPanels[0].metrics[3].value, "24.5");
  assert.equal(summary.chartPanels[1].periodLabel, "最近 30 天");
  assert.deepEqual(
    summary.chartPanels[1].metrics.map((metric) => metric.label),
    ["每日跑量", "跑步次数", "平均配速", "平均心率"],
  );
  assert.equal(summary.chartPanels[1].metrics[0].points.length, 30);
  assert.equal(summary.chartPanels[1].metrics[0].points.at(-1).value, 5);
  assert.equal(summary.chartPanels[1].metrics[2].value, "6");
});
