...
location / {
  root /www/wwwroot/personal-site/current/frontend;
...
- 已建立 `frontend/` Astro + Vue 静态前端骨架。
- 已建立 `/`、`/projects/slimming/`、`/app/slimming/` 三个静态页面。
- 已建立 `backend/` Go API 骨架，当前提供 `/healthz` 和 `/api/healthz`。
...
阶段 1 完成后，再开始搭建 `frontend/` 和 `backend/` 的新技术栈骨架。
